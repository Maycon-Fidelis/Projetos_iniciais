...
while rodando:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and len(posicoes) < 7 and pontosCaptura == True:
        ...
        for ponto in posicoes:
            pygame.draw.circle(screen,AZUL,ponto,10)

        if not pontosCaptura:
            scanline(vertices)
            pygame.draw.polygon(screen,AZUL,vertices,1)
